...
def test_customdata_tooltip(dash_duo):
    ...
    WebDriverWait(dash_duo.driver, 10).until(EC.presence_of_element_located((By.ID, GRAPH_ID)))

    # Click the first data point
    first_point = WebDriverWait(dash_duo.driver, 10).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, ".scatterlayer .trace .points path:nth-of-type(1)"))
    )
    ...
    WebDriverWait(dash_duo.driver, 10).until(
        lambda driver: (
            driver.find_element(By.CSS_SELECTOR, "g.annotation-text-g text.annotation-text").text.strip() != ""
        )
    )
    ...
    WebDriverWait(dash_duo.driver, 10).until(
        lambda driver: len(driver.find_elements(By.CSS_SELECTOR, "g.annotation-text-g text.annotation-text")) >= 2
    )

    # Use JavaScript to retrieve all tooltip texts
    tooltip_texts = dash_duo.driver.execute_script(
        "return Array.from("
        "document.querySelectorAll('g.annotation-text-g text.annotation-text')"
    ...
    assert "Data Points,x: 1,y: 2.00" in tooltip_texts[0], f"First tooltip content does not match: {tooltip_texts[0]}"
    assert "UPDATED,x: 2,y: 3.00" in tooltip_texts[1], f"Second tooltip content does not match: {tooltip_texts[1]}"
